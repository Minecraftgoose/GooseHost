        ...
        // JS 字符串 + HTML 属性双重转义：用于 onclick="fn('...')" 这类内联合法参数
        function jsStr(str) {
            ...
            try {
                ...
                list.innerHTML = `<div class="card"><div class="site-list">` +
                    sites.map((site, i) => {
                        ...
                        return `
                        <div class="site-item splash-action" data-slug="${escapeHtml(site.name)}" onclick="showSiteDetail('${attrJs(site.name)}')" style="animation-delay: ${(window.__splashDone ? 0 : 2.5) + i * 0.08}s">
                            <div class="site-info">
                ...
                document.getElementById('pageDetailBody').innerHTML = `
                    <div class="detail-head">
                        ...
                        <div class="detail-actions">
                            ...
                            ${siteType !== 'project' ? `<button class="btn" onclick="editSite('${attrJs(s.name)}','${s.type}');return false;"><i class="fas fa-edit"></i> 编辑</button>` : ''}
                            <button class="btn btn-danger" onclick="confirmDelete('${attrJs(s.name)}');return false;"><i class="fas fa-trash"></i> 删除</button>
                        </div>
                    </div>
                    ...
                    <div class="detail-files">
                        ${data.files.map(f => `<div class="detail-file${siteType === 'project' ? ' proj-file-row' : ''}" data-path="${escapeHtml(f.name)}">
                            ...
                            <span class="file-actions">
                                <button onclick="editProjectFile('${attrJs(s.name)}','${attrJs(f.name)}')"><i class="fas fa-edit"></i> 编辑</button>
                                <button onclick="deleteProjectFile('${attrJs(s.name)}','${attrJs(f.name)}')" class="del"><i class="fas fa-trash"></i> 删除</button>
                            </span>` : ''}
                        </div>`).join('')}
                    </div>`;
